...
from backbone.consensus import proofOfWork
...
def main(argv):
    try:
        ...
        for opt, arg in opts:
            ...
            if opt == "-m":  # mine block
                block = proofOfWork()
                searalizedData = block.to_dict()
                response, _, _ = flask_call('POST', BLOCK_PROPOSAL, data=searalizedData)
                print(response)
                valid_args = True
            if opt == "-i":
                # INFO
                if arg == "b":
                    ...
                    if users:
                        for i in users:
                            # print username and total reward. 
                            print(i['username'], i['total_reward'])
                    valid_args = True
                else:
                    valid_args = False
            if opt == "-t":
